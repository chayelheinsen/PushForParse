...
public class Ionicons: NSObject {
    ...
    public class func imageWithIcon(iconName:Ionicon, size: CGFloat, color: UIColor) -> UIImage {
        ...
        return Ionicons.imageWithIcon(iconName,  iconSize:size,  iconColor: color, imageSize: CGSizeMake(size, size));
        
    }
    
    public class func imageWithIcon(iconName:Ionicon, iconSize: CGFloat, iconColor: UIColor = UIColor.blackColor(), imageSize: CGSize) -> UIImage {
        ...
        let attString = NSAttributedString(string: iconName.rawValue, attributes: [NSFontAttributeName:  Ionicons.fontWithSize(iconSize), NSForegroundColorAttributeName: iconColor, NSParagraphStyleAttributeName: style])
